...
AutoSnippet 是一个本地化工具，用户 `npm install -g autosnippet` 后就应该能用。这意味着不能依赖 PostgreSQL、Redis、Elasticsearch 等外部服务。但系统需要：关系型存储（知识条目 · 审计日志 · 15 张表）、向量存储（语义搜索 · HNSW 索引）、缓存（AST 图谱 · 搜索结果 · 跨进程失效）、事件总线（信号分发）、依赖注入（70+ 服务管理）。
...
AutoSnippet 的 SQLite 数据库包含 15 张表，用 Drizzle ORM 定义 schema，编译期保证列名和类型的正确性。以下是核心表的设计：
...
- **SQLite + WAL** 提供关系存储，15 张表覆盖知识、分析、审计、会话四个领域
