...
更关键的是：这个引擎不能只为一种任务服务。聊天场景需要 8 轮以内的快速交互，深度分析需要 24 轮的多阶段流水线，飞书消息需要 60 秒超时的轻量执行。这些看似不同的任务，底层用的是**同一个循环引擎**——通过 Preset 配置来改变行为。
...
**为什么不用简单的 Chain？** Chain（如 LangChain 的 SequentialChain）是预定义的线性流程——A → B → C。但知识分析任务本质上是**探索性的**：Agent 不知道要搜索几次、不知道哪些文件有价值、不知道什么时候"够了"。ReAct 循环允许 Agent 在运行时动态决定下一步，而不是编译时固定步骤。
  ...
  while true:
    ...
    try:
      llmResult ← aiProvider.chatWithTools(prompt, {
        messages, systemPrompt, toolSchemas, toolChoice
      })
    ...
    if llmResult.functionCalls:
      ...
      tracker?.endRound({newInfo, submitCount, toolNames})
      continue  // 下一轮迭代
...
**`cleanFinalAnswer`**。LLM 返回的文本可能包含内部推理标记（如 `<think>...</think>`）。`cleanFinalAnswer` 剥离这些标记，只保留面向用户的内容。
...
class LoopContext {
  ...
  sharedState?: { submittedTitles, submittedPatterns };  // 提交去重
...
| **最大迭代** | `iteration ≥ maxIterations + graceRounds` | 预算耗尽 |
| **超时** | `Date.now() - startTime ≥ timeoutMs` | 长时间无进展 |
| **空闲停滞** | `roundsSinceNewInfo ≥ idleRoundsToExit` | 连续 3 轮无新信息 |
...
| insight | 24 | 300s | 深度分析 + 知识提取 |
| lark | 6 | 60s | 飞书消息桥接 |
| remote-exec | 6 | 60s | 终端命令执行 |

### ExplorationTracker：阶段状态机

系统发起的循环（insight/bootstrap）使用 `ExplorationTracker` 来控制**阶段转换**。Tracker 把循环划分为三个阶段：

```
SEARCH → VERIFY → SUMMARIZE
```
...
|:---|:---|:---|:---|
| **SEARCH** | `auto` | 自由搜索，发现文件和模式 | searchBudget 耗尽 |
| **VERIFY** | `auto` | 停止新搜索，确认关键细节 | 证据充分 |
...
关键细节：`rollbackTick()` 把 Tracker 的迭代计数器回退一步。因为空响应不是 Agent "选择了什么"，而是 LLM 的故障——不应该消耗宝贵的迭代预算。
...
在知识生产循环中（insight/bootstrap preset），Agent 可能反复尝试提交相同或高度相似的知识候选。`sharedState` 中维护了两个 Set：

- `submittedTitles: Set<string>` — 已提交的标题
- `submittedPatterns: Set<string>` — 已提交的模式签名

`ToolExecutionPipeline` 的去重中间件在执行 `submit_knowledge` 工具前检查这两个 Set——如果标题或模式已存在，返回错误消息而不实际调用工具：

```json
{ "error": "Duplicate: already submitted with title '...'" }
```
...
不同模型的上下文窗口差异巨大（Gemini 3 有 1M，GPT-4 有 128K，小模型可能只有 32K）。`ContextWindow` 根据模型名称自动调整预算：
...
static resolveTokenBudget(modelName, opts) {
  ...
  if (contextSize >= 200_000) {
    return opts.isSystem ? 32_000 : 24_000;
  } else if (contextSize >= 64_000) {
    return opts.isSystem ? 24_000 : 20_000;
  } else {
    return Math.floor(contextSize * 0.7);
  }
}
```

系统模式比用户模式多分配 33% 的预算——因为系统循环（如 insight）执行更多工具调用，需要更大的上下文空间来容纳中间结果。
...
| **insight** | Code Analysis, Knowledge Production | Pipeline（Analyze → Gate → Produce → Gate） | 24 轮 / 300s | 深度分析 + 知识提取 |
| **lark** | Conversation, System Interaction | Single | 6 轮 / 60s | 飞书命令桥接 |
| **remote-exec** | Conversation, System Interaction | Single | 6 轮 / 60s | 终端命令执行 |
...
Stage 1: Analyze（分析）
  ...
  Budget: 24 轮, temperature 0.4, timeout 300s
  SystemPrompt: "遍历项目架构，找出核心模块和交互模式"
  Phase: SEARCH → VERIFY → SUMMARIZE
      ↓
...
Stage 3: Produce（生产）
  ...
  Budget: 20 轮, temperature 0.3
  SystemPrompt: "基于分析输出候选知识条目"
  Phase: PRODUCE → SUMMARIZE
      ↓
...
| 评估结果 | 条件 | 动作 |
|:---|:---|:---|
| **pass** | 证据 > 500 字符 + 文件引用 ≥ 3 | 继续到 Produce 阶段 |
| **retry** | 证据不足但有基础 | 重新执行 Analyze（最多 1 次） |
| **degrade** | 证据极少（< 200 字符） | 跳过 Produce，直接输出摘要 |
...
LLM_CLASSIFICATION_SCHEMA = {
  preset: 'chat' | 'insight' | 'lark' | 'remote-exec',
...
2. AgentFactory → PipelineStrategy, Budget={iter:24, timeout:300s}

3. Stage 1 — Analyze:
   reactLoop(SEARCH phase):
     Iter 1-6: search_code, list_files, read_file → 发现 15 个文件
     ExplorationTracker: searchBudget 耗尽 → VERIFY nudge
   reactLoop(VERIFY phase):
     Iter 7-9: 确认关键类的继承关系和协议实现
...
Chat 场景设 8 轮是因为用户期望快速响应——等 3 分钟得到一个更完整的答案，不如等 10 秒得到一个 80% 质量的答案。Insight 场景设 24 轮是因为它通常在后台执行（Bootstrap），用户对延迟容忍度高。
