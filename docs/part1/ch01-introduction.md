...
AutoSnippet 不是一个"让 LLM 读代码然后总结"的脚本，而是一个完整的知识工程系统。它让编程 Agent 通过 MCP 工具与项目知识库语义对话——发问与回答在同一个 Agent 内完成，先提交有限的答案，用来回答无限的问题。以下是各核心模块的速览，每个模块在后续章节中都有独立的深入解析。
