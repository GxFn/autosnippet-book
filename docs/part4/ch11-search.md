...
解决方案是 **Weighted-First + Confidence Gate**——先执行 weighted 搜索（~40ms），根据结果的"置信度"决定是否值得再等 embed：
...
if (confidence >= 65) {
  ...
  results = scorerItems;
  actualMode = 'auto(weighted-skip)';
} else {
  // weighted 不确定 → 调用 embed 做 RRF 融合
  const rrfResults = await vectorService.hybridSearch(query, { ... });
  // ...
}
```

置信度由三个正向信号叠加，两个负向信号抑制：
...
| 中文疑问句 | 置零 | 包含"如何/怎样/什么/为什么" → 一定走语义 |
...
"Cookie"                 → conf 95, skip     (35ms)
"@video-player-reuse"    → conf 95, skip     (38ms)
"如何在列表页复用播放器"  → conf  0, invoke   (11.7s)
"network middleware"     → conf  0, invoke   (3.7s)
"Conventional Commits"   → conf  0, invoke   (9.7s)
```

路由准确率在 BiliDili 知识库的全部测试查询中达到 100%——所有精确查询被正确跳过，所有语义查询走了 embed。平均延迟从 ~10 秒降到了 ~2 秒（大多数 MCP 搜索请求是精确术语查询）。
...
| 阶段 | 耗时 | 占比 |
|:---|:---|:---|
| FieldWeighted 全流程 | 35–40ms | — |
| Embedding API 调用 | 2.3–22.3s（中位 ~6s） | 99.5% |
| HNSW 图搜索 | 8–19ms | 0.3% |
| RRF 融合 + 元数据补全 | 5–15ms | 0.2% |

HNSW 在 ~50 条规模上几乎零开销。**语义搜索的全部瓶颈是 Embedding API 的网络延迟**——它极不稳定（2s 到 22s），偶发 `fetch failed` 时触发重试，最差可达 40 秒后降级到纯 weighted。
...
**auto (RRF) 的陷阱**：当前融合权重 0.6:0.4 偏向 sparse，导致 dense 信号几乎被淹没——11 组测试中 auto 的 Top-5 排序与纯 weighted **完全一致**。semantic 找到的独有好结果被 sparse 的高排名压制，融合后沉到了看不见的位置。如果要让语义搜索真正影响融合排名，alpha 需要提高到 0.5–0.6。
...
2. **延迟不可控**。实测 Embedding API 延迟从 2.3 秒到 22.3 秒不等，中位数约 6 秒——而 HNSW 图搜索本身只需 8–19ms。语义搜索 99.5% 的时间在等网络。对比之下 weighted 恒定 35–40ms。在 MCP 场景下，AI Agent 等 6 秒获得的结果质量提升（如果有的话）往往不值这个延迟代价。
...
2. **路由优于蛮力**。不是每次搜索都需要调用 Embedding API——Weighted-First Confidence Gate 用 40ms 的预判省去了大多数场景下 2–22 秒的 embed 等待。真实测试表明，精确查询占 MCP 搜索请求的大多数，路由后平均延迟从 ~10 秒降到 ~2 秒。
