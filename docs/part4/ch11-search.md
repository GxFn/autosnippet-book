...
AutoSnippet 的搜索引擎提供五种模式（`auto`、`keyword`、`weighted`、`semantic`、`context`），但核心设计思想是**三种召回策略的统合**：

| 策略 | 强项 | 弱项 |
|:---|:---|:---|
| **BM25 关键词** | 精确匹配"dispatch_sync"一定能找到 | "网络请求"找不到 "HTTP call" |
| **向量语义** | "网络请求"能匹配到 "HTTP call" | "dispatch_sync"精确匹配时反而分数不高 |
| **字段加权** | trigger `@xxx` 精确定位、title 高权重命中 | 纯语义查询力不从心 |

三种策略各有盲区——只用一种永远有漏洞。AutoSnippet 用 **RRF（Reciprocal Rank Fusion）** 把三种策略的排名融合为一个统一分数：
...
RRF 的优雅之处在于它**只用排名，不用分数**。BM25 的分数范围可能是 0–15，向量相似度的范围是 0–1——两者直接相加毫无意义。但排名是可比的：BM25 排第 1 + 向量排第 3 = 融合后排名很高。$k=60$ 控制平滑度——$k$ 越大，排名差距被压缩得越多，前几名的优势越不明显。

```
例: 
  Recipe A: BM25 rank 1, Vector rank 5
  RRF = 0.5 × 1/(60+1) + 0.5 × 1/(60+5)
      = 0.5/61 + 0.5/65
      ≈ 0.0082 + 0.0077 = 0.0159

  Recipe B: BM25 rank 10, Vector rank 1
...
传统 BM25 的做法是把所有字段拼接为一段文本，然后整体计分。但对于 AutoSnippet 的知识库来说这行不通——BM25 把字段拼接后做 tokenize 去重，导致 TF（词频）永远为 1，BM25F 的字段 boost 完全失效。
...
召回阶段解决的是"找到相关内容"——但相关性只是排序的维度之一。`MultiSignalRanker` 用六个信号对候选结果进行精排：
...
六个信号归一化到 [0, 1] 后，根据**场景**分配不同权重。同一条 Recipe，在 lint 场景和 learning 场景中的排名可能完全不同：
...
召回 (FieldWeighted + Vector, RRF 融合)
  ...
  │
  ├── Level 3: MultiSignalRanker 精排  ← 六信号
  │     场景化权重 × 归一化信号
  │
...
| 维度 | 默认权重 | 数据源 |
|:---|:---|:---|
| recall | 0.45 | FieldWeighted/BM25 召回分 |
| semantic | 0.30 | 向量相似度 |
...
**Level 3: MultiSignalRanker 六信号精排**（前文已详述）
...
六个信号 × 五种场景 = 30 个权重参数。这看起来是一个调参噩梦。实际上，这些权重不需要精确优化：
...
1. **融合优于单一**。没有哪种搜索策略能覆盖所有场景——BM25 精确但不理解语义，向量理解语义但不擅长精确匹配。RRF 用排名取代分数做融合，优雅地回避了不同策略之间分数不可比的问题。
